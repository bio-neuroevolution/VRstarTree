...
class MerkleKDTree():
    ...
    def __init__(self, data,pointonly=True):
        self.depth = 0
        ...
        def boundary(point,axis,direction):
            if point[3] == 0:return point[axis]
            else: return point[axis] + direction * point[4]/2
        # print('dim', dim, 'tm_axis', tm_axis)
        def build_kdtree(point_list, depth,pointonly=True):
            ...
            if len(point_list)==1:
                ...
                return KDTreeNode(point=point_list[0],left=None,right=None)
            ...
            else: # 增加了在KD树上存储立方体数据
                median = len(point_list) // 2  # choose median
                left = sorted([p for p in point_list if p!=point_list[median] and boundary(p, axis, -1) < point_list[median][axis]],
                              key=lambda point: point[axis])
                right =sorted([p for p in point_list if p!=point_list[median] and boundary(p, axis, 1) > point_list[median][axis]],
                               key=lambda point: point[axis])
                #print('MKDTree:'+str(len(left))+"+"+str(len(right)) + "=" + str(len(left)+len(right)) + "-----" + str(len(point_list)))
                #others = list(set(point_list) - set(left) - set(right) - set([point_list[median]]))
                #left = sorted(left + others,key=lambda p:p[axis])
                #right = sorted(right+others,key=lambda p:p[axis])

                node = KDTreeNode(point=point_list[median],
                                  left=build_kdtree(left, depth + 1,pointonly),
                                  right=build_kdtree(right, depth + 1,pointonly))
            return node

        self.dim = dim
        self.root_node = build_kdtree(data, depth=0,pointonly=pointonly)
    ...
    def compute_merkle_proof(self):
        def combine_and_hash(a, b):
            return hasher.sha256((a+b).encode('utf-8')).hexdigest()

        def leveled_hash(node, depth):
            if node.is_leaf():
                return node.point_hash

            l_hash_pair = leveled_hash(node.left, depth+1)
            r_hash_pair = leveled_hash(node.right, depth+1)
            node.point_hash = combine_and_hash(l_hash_pair, r_hash_pair)
            return node.point_hash

        merkle = leveled_hash(self.root_node, 0)
        return merkle

    def proof_length(self,unit=64):
        """
        提供证明串长度
        """
        #hash = self.compute_merkle_root()
        return self.depth * unit


    
    @staticmethod
    def construct_from_data(data,pointonly=True):
        tree = MerkleKDTree(data,pointonly)
        return tree
    ...
    def _range(self, node, depth, min_point, max_point):
        ...
        for i, coord in enumerate(node.point[:3]):
            if length == 0:
                if not min_point[i] <= coord <= max_point[i]:
                    return points
            else:
                if max_point[i] < coord - length/2 or min_point[i] > coord + length/2:
                    return points
