...
class MerklePatriciaTree:
    ...
    def __init__(self, xh=1,from_scratch=None, debug=None):
        if debug:
            self.debug = debug
        if from_scratch:
            self.db = LvlDB(xh=xh,debug=self.debug, from_scratch=True)
            self.root_hash = self.init_new_root()
        else:
            self.db = LvlDB(xh=xh,debug=self.debug, from_scratch=False)
            try:
    ...
    def proof_length(self,count=1,unit=64):
        """
        采样证明长度
        :param count int 采样次数
        :param unit  int 每个节点的证明长度
        """
        lengths = []
        for i in range(count):
            length = 0
            node = pickle.loads(self.db.get(self.root_hash))
            while node is not None:
                if isinstance(node,LeafNode):
                    ...
                    length += unit
                    if node.child_hash is None or len(node.child_hash)<=0:
                        break
                    node = pickle.loads(self.db.get(node.child_hash))
                    continue
                elif isinstance(node,BranchNode):
                    length += (node.size()-1)*unit
                    index = int(np.random.random()*node.size())
                    node = pickle.loads(self.db.get(node.index(index)))
