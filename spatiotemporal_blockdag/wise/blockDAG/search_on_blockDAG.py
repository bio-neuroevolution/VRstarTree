...
def kd_range(self, min_point, max_point, t_start, t_end=None):
    ...
    for indx in kdtree_indxes_gen(self, t_start, t_end):
        r = self.merkle_kd_trees[indx].range(min_point, max_point)
        r = [d for d in r if not d in results]
        results += r

    return results

def _kd_in(point,results):
    for r in results:
        if abs(r[0] - point[0])<=1 and abs(r[1] - point[1])<=1 and abs(r[2] - point[2])<=1:
            return True
    return False
