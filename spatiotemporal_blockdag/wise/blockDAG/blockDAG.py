...
class BlockDAGlocal:
    ...
    def create_block(self, proof,pointonly=True):
        """
        ...
        if self.current_transactions:
            ...
            tree, m_root = self.form_merkle_kdtree(self.current_transactions,pointonly)
    ...
    def new_transaction(self, account, lng, lat, ts,geotype,length):
        """
    ...
    def form_merkle_kdtree(self, current_transactions,pointonly=True):
        data = pd.DataFrame(current_transactions)
        data_to_bulk = [(*to_Cartesian(row[0], row[1]), row[2],row[3],row[4]) for row in data[['lat', 'lon', 'ts','geotype','length']].values]
        tree = MerkleKDTree.construct_from_data(data_to_bulk,pointonly)
